---
title: "Supported LLM Providers and How to Configure Them"
sidebarTitle: "Overview"
description: "Supported AI providers in GoModel and which ones have dedicated configuration guides."
icon: "list"
keywords: ["AI providers", "supported providers", "LLM providers", "provider list", "configure provider"]
---
...
(`api_keys`, an ordered list) and the per-provider
[outbound proxy](/providers/outbound-proxy) are available from the dashboard
form.
...
- **Z.ai reasoning effort** — `reasoning.effort` (Chat Completions and
  Responses) is sent as Z.ai's top-level `reasoning_effort`. GLM-5.3 and
  GLM-5.3-Flash always think and accept only `low`, `high`, and `max`, so
  `none` and `minimal` map to `low`, `medium` to `high`, and `xhigh` to `max`.
  Other GLM models receive the level unchanged.
...
- **vLLM** — set `VLLM_API_KEY` only if the upstream server was started with
  `--api-key`.
- **audio.cpp** — `AUDIOCPP_BASE_URL` is required (audiocpp_server's default
  port collides with GoModel's own 8080, so there is no default);
  `AUDIOCPP_API_KEY` is usually unset, since audio.cpp has no authentication of
