...
| Kimi | `KIMI_API_KEY` (`KIMI_BASE_URL` optional) | `kimi-k2-0711-preview` | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | [Kimi](/providers/kimi) |
...
- **vLLM** — set `VLLM_API_KEY` only if the upstream server was started with
  `--api-key`.
- **Per-provider header customization** — each provider block in `config.yaml`
  accepts two optional fields that control outbound headers: `custom_headers`
  (a static map of header name to value, sent on every request to that
  provider) and `passthrough_user_headers` (a boolean that copies inbound
  request headers onto the outbound request, after a small skip list for
  credential and transport-managed headers). By default every provider sets
  `passthrough_user_headers` to `false`; **Kimi is the one exception** and
  defaults it to `true`. See the [Kimi provider page](/providers/kimi) for the
  full example, and the [Configuration reference](/advanced/configuration) for
  the matching env-var details. `custom_headers` is YAML-only — there is no
  environment-variable equivalent; use `<PROVIDER>_PASSTHROUGH_USER_HEADERS`
  to toggle the passthrough switch from env.
