...
`GET /v1/models` returns whatever the account's plan exposes. Pin a static
list instead when you want a fixed surface:

```bash
CURSOR_MODELS=composer,auto
```

Which slugs exist depends on the subscription tier. The **Cursor Models
pool** (generous included usage) currently includes Composer 2.5 and Grok
4.5 / 4.6 on Pro and above; a Hobby or trial account may only expose
`composer` / `auto`. Confirm with a live `GET /v1/models` against your key.
