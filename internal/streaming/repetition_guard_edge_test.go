...
// Edge-case tests covering real traffic shapes: CRLF separators, usage final
// chunks, long repeating units on models the tokenizer cannot resolve, and
// reasoning_content-only streams. The reasoning_content case pins the current
// contract (inspected) so the behavior stays explicit.
...
// to 64 bytes, a 40-byte repeating unit on an unresolvable model must still
// trip the guard; units above the cap stay undetected by design.
